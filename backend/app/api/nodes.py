...
async def register_node(payload: NodeCreate, session=Depends(get_db)):
...
async def list_nodes(session=Depends(get_db)):
    service = NodeService(session)
...
async def update_node(node_id: int, payload: NodeUpdate, session=Depends(get_db)):
    service = NodeService(session)
    try:
...
async def delete_node(node_id: int, session=Depends(get_db)):
    service = NodeService(session)
    try:
