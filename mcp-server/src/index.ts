    ...
    tools: [
      {
        ...
        description:
          "Ingest a ChatGPT or Claude conversation into the Glyph Foundry knowledge graph. Creates glyphs for conversation turns and generates knowledge relationships.",
        inputSchema: {
          type: "object",
          properties: {
            conversation_data: {
              type: "string",
              description:
                "JSON string containing conversation data with conversationId, messages array, and optional metadata",
            },
            source: {
              type: "string",
              enum: ["chatgpt", "claude"],
      ...
      },
      {
        ...
        description:
          "Create 4D glyphs from conversation metrics (turn count, message length, sentiment, etc.)",
        inputSchema: {
          type: "object",
          properties: {
            conversation_id: {
      ...
      },
      {
        ...
        description:
          "Analyze conversation patterns and generate insights for the knowledge graph",
        inputSchema: {
          type: "object",
          properties: {
            conversation_id: {
...
async function ingestConversation(args: any) {
  const { conversation_data, source } = args;

  const conversation: ConversationData = JSON.parse(conversation_data);
...
async function createConversationGlyphs(args: any) {
  const { conversation_id, metrics } = args;

  const glyphMetrics: GlyphMetric[] = metrics.map((m: any) => ({
    metric_name: m.name,
...
async function analyzeConversation(args: any) {
...
async function sendToGlyphAPI(metrics: GlyphMetric[]): Promise<void> {
