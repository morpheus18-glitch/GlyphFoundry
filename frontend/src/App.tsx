...
export default function App() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gradient-to-b from-[#0a0a0f] via-[#050508] to-black text-gray-100">
      <header className="absolute top-0 left-0 right-0 z-50 bg-gradient-to-b from-black/80 via-black/60 to-transparent backdrop-blur-xl border-b border-cyan-500/10 shadow-2xl shadow-cyan-500/5">
        <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4 px-3 md:px-6 py-3 md:py-4">
          <div className="flex items-center gap-3 md:gap-4">
            <div className="relative h-10 w-10 md:h-12 md:w-12 rounded-xl bg-gradient-to-br from-cyan-400 via-purple-500 to-fuchsia-600 flex items-center justify-center text-white font-black text-lg md:text-xl shadow-2xl shadow-cyan-500/50 animate-pulse">
            ...
            <div className="flex flex-col">
              <strong className="text-xl md:text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 via-purple-300 to-fuchsia-300 tracking-wider">
                GLYPH FOUNDRY
              </strong>
              <span className="text-[9px] md:text-[10px] uppercase tracking-[0.25em] md:tracking-[0.3em] text-cyan-500/60 font-semibold">Cinematic Knowledge OS</span>
            </div>
          </div>
          ...
          <nav className="flex flex-wrap gap-2 md:gap-3">
            {navItems.map(([mode, label]) => (
              <button
                ...
                className={`relative overflow-hidden rounded-lg md:rounded-xl px-3 md:px-5 py-1.5 md:py-2.5 text-xs md:text-sm font-bold uppercase tracking-wider transition-all duration-300 ${
                  view === mode 
            ...
            {!loading && graph && (
              <div className="flex items-center gap-2 md:gap-4 text-xs md:text-sm font-semibold">
                <span className="flex items-center gap-1.5 md:gap-2 px-2 md:px-3 py-1 md:py-1.5 rounded-lg bg-cyan-500/10 border border-cyan-500/30">
                  ...
                  <span className="text-cyan-500/60 hidden sm:inline">Nodes</span>
                </span>
                <span className="flex items-center gap-1.5 md:gap-2 px-2 md:px-3 py-1 md:py-1.5 rounded-lg bg-purple-500/10 border border-purple-500/30">
                  ...
                  <span className="text-purple-500/60 hidden sm:inline">Links</span>
                </span>
              </div>
            )}
            {!loading && !graph && (
              <span className="text-xs md:text-sm text-gray-600 font-semibold">Initializing...</span>
            )}
          </div>
          {error && (
            <span className="rounded-lg md:rounded-xl border border-red-500/50 bg-red-500/10 px-3 md:px-4 py-1.5 md:py-2 text-xs md:text-sm text-red-300 font-semibold backdrop-blur-sm" role="alert">
              {error}
            </span>
          )}
        </div>
