...
export class PhysicsEngine {
     ...
     */
    setNodes(nodes_js) {
        const ret = wasm.physicsengine_setNodes(this.__wbg_ptr, nodes_js);
        if (ret[1]) {
     ...
     */
    setEdges(edges_js) {
        const ret = wasm.physicsengine_setEdges(this.__wbg_ptr, edges_js);
        if (ret[1]) {
            throw takeFromExternrefTable0(ret[0]);
        }
    }
     ...
     * @param {number} theta
     */
    setParams(repulsion, attraction, damping, theta) {
        wasm.physicsengine_setParams(this.__wbg_ptr, repulsion, attraction, damping, theta);
    }
    /**
     ...
     */
    tick(delta_time) {
        const ret = wasm.physicsengine_tick(this.__wbg_ptr, delta_time);
        if (ret[2]) {
     ...
     */
    getNodes() {
        const ret = wasm.physicsengine_getNodes(this.__wbg_ptr);
        if (ret[2]) {
            throw takeFromExternrefTable0(ret[1]);
        }
        return takeFromExternrefTable0(ret[0]);
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('glyph_physics_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
